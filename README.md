...
> ⚠️ **Do not close the terminal** — Tomcat runs inside it.
...
| Error | Fix |
|-------|-----|
| `JAVA_HOME not defined` | Open Admin PowerShell and run: `[System.Environment]::SetEnvironmentVariable("JAVA_HOME","C:\Program Files\Java\jdk-25","Machine")` |
| `Database error` | Open XAMPP Control Panel → Start MySQL |
| `ERR_FAILED / Port 8080 in use` | Run `Get-Process -Name "java" \| Stop-Process -Force` then re-run `.\run.ps1` |
| `Access denied for root` | Check that `DB_PASS = "root"` in `DBConnection.java` |
...
```powershell
# Bypassing complex flags using the built-in run command:
...
© Copyright. All Rights Reserved. Ramai Kulkarni.
